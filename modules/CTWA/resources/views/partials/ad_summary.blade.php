@extends('layouts.app', ['title' => __('Lead Campaign Summary')])
    ...
    <div class="row g-4">
            ...
            <div class="card summary-card h-100">
                ...
                <div class="card-body">
                    ...
                    <p><span class="summary-title">Age Group</span><br>
                       <span class="summary-value">{{ isset($summary['age_from'], $summary['age_to']) ? $summary['age_from'] . ' ~ ' . $summary['age_to'] : 'N/A' }}</span>
                    </p>
            ...
            <div class="row g-4">
                ...
                <div class="col-6 col-md-3 mt-2">
                    <div class="card summary-card h-100 text-center">
                        <div class="card-body">
                            <div class="text-muted"><i class="bi {{ $metric['icon'] }} metric-icon"></i>{{ $metric['label'] }}</div>
            ...
            <div class="card summary-card mt-4">
                <div class="card-body">
                    ...
                    <div class="row g-3 align-items-center mb-4">
                        <div class="col-md-4">
                            <select id="preset-range" class="form-select form-control">
                                <option value="7" selected>Last 7 days</option>
                                <option value="14">Last 14 days</option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            <input type="date" id="start-date" class="form-control" value="{{ now()->subDays(7)->toDateString() }}">
                        </div>
                        <div class="col-md-4">
                    ...
                    <div class="row g-4 text-center" id="leads-summary">
                        <div class="col-md-4"><div class="lead-box"><div class="summary-title">Total</div><div class="summary-value">{{ count($leads) }}</div></div></div>
                        <div class="col-md-4"><div class="lead-box success"><div class="summary-title">New</div><div class="summary-value">-</div></div></div>
                        <div class="col-md-4"><div class="lead-box warning"><div class="summary-title">Existing</div><div class="summary-value">-</div></div></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        @if($CTWAAdsClickLead->count())
            <div class="card mt-4 col-12">
                ...
                <div class="mt-3">
                    <button class="btn btn-primary mt-3" data-bs-toggle="modal" data-bs-target="#broadcastModal">📢 Broadcast</button>
                </div>
                        ...
                        <tbody>
                            @foreach($CTWAAdsClickLead as $index => $lead)
                                @php
                                    $url = $lead->source_url;
                                    $platform = 'Unknown';
                                    if (Str::contains($url ?? '', 'instagram.com')) $platform = 'Instagram';
                                    elseif (Str::contains($url ?? '', ['fb.me', 'facebook.com'])) $platform = 'Facebook';
                                @endphp
                                <tr>
                                    <td>
                                        <input type="checkbox" class="leadCheckbox" data-lead='@json(["wa_id"=>$lead->wa_id,"contact_id"=>$lead->contact_id,"company_id"=>$lead->company_id])'>
                                    </td>
                                    <td>{{ $index + 1 }}</td>
...
<div class="modal fade" id="broadcastModal" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form id="campaignForm" action="{{ route('campaign.send') }}" method="POST">
        @csrf
